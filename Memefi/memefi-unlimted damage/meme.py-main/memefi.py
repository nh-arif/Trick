...
# Function to restart the program after an exception with a countdown
def restart_program():
    print("❌ Error occurred. Restarting the program in 10 seconds...")
    for i in range(10, 0, -1):  # 10 seconds countdown
...
def boss_wait():
    print("Boss Defeated wait 5 seconds..")
    for i in range(5, 0, -1):  # 5 seconds countdown
        mins, secs = divmod(i, 60)
        timer = '{:02d}:{:02d}'.format(mins, secs)
        print(f"\rCountdown: {timer}", end="", flush=True)
        time.sleep(1)  # Wait for 1 second
...
# Function to handle Turbo Booster activation with exception handling and restart
def activate_booster(index, headers):
    try:
        ...
        headers = headers_set.copy()  # Create a copy of headers to avoid modifying the global one
        headers['Authorization'] = f'Bearer {access_token}'
        ...
        response = safe_post(url, headers, booster_payload)
        if response and 'data' in response:
            ...
            if current_health == 0:
                print("\nBoss defeated, setting next boss...")
                ...
                # Perform taps and process results
                for _ in range(50):
                    ...
                    if tap_result is not None:
                        if 'data' in tap_result and 'telegramGameProcessTapsBatch' in tap_result['data']:
                            ...
                            if(tap_data['currentBoss']['currentHealth'] == tap_data['currentBoss']['maxHealth']):
                                boss_wait()
                    else:
                        print(f"❌ Failed to submit taps, retrying...")
        else:
            print(f"❌ Failed with status {response}, retrying...")

    # Catch exception and restart the program after waiting 15 minutes
...
# Set default values directly instead of requiring input
cek_task_enable = 'n'
auto_booster = 'y'  # Default to 'y' for Energy Booster
turbo_booster = 'y'  # Default to 'y' for Turbo Booster
god_mode = 'y'  # Default to 'y' for God Mode (1x tap monster dead)
auto_claim_combo = 'y'  # Default to 'n' for Auto claim daily combo

# If needed, set a default combo
if auto_claim_combo == 'y':
    vector = '4,2,4,2'  # Default combo if auto claim combo is enabled
